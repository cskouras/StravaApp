...
def connected_map(request):
    ...
    activity_df_list = []
    for n in range(10):  # Change this to be higher if you have more than 2000 activities
        param = {'per_page': 200, 'page': n + 1}
        # get the activities from GET request 
    ...
    # # Get workout totals 
    # for activity in activity_df_list[:20]:
    #     print(type(activity))
    #     print(activity['type'])
    #     logging.debug(activity['type'])
    #     if activiy
    ...
    for index, activity in activities_df.iterrows():
        pl = activity['polylines']
        if activity['type'] == "Run":
            color = 'red'
        elif activity['type'] == "Rowing":
            color = 'blue'
        elif activity['type'] == "Biking":
            color = 'green'
        elif activity['type'] == "Skiing":
            color = 'black'
        else: 
            color = 'yellow'

        if len(pl) > 0: # Ignore polylines with length zero (Thanks Joukesmink for the tip)
            folium.PolyLine(locations=pl, color=color).add_to(main_map)
